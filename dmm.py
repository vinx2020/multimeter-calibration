...
data_c2 = 0
thread = 0
protokol = r'C:\\Users\\vgatto\\cernbox\\02_Python\\TestPoint2\\DMM\\Protocol' 
shablon = r'C:\\Users\\vgatto\\cernbox\\02_Python\\TestPoint2\\DMM\\Shablon'
            ...
            while i < v - 1:
                ...
                if re.search(self.adr, rg1[i]):
                    ...
                    if a1[2] in ('34410A', '34411A', '34460A', '34461A', '34465A', '34470A'):
                        a10.set('Multimeter ' + a1[2] + ' ' + 'connected')
                        lb.insert('end', a10.get())
                        lb.see('end')
                    if a1[2] in ('33622A', 'N5183A'):
                        a10.set('Generator ' + a1[2] + ' ' + 'connected')
                        lb.insert('end', a10.get())
                        lb.see('end')


        if combo1.get()[:4] == self.type:
            inst_1 = rm.open_resource(combo1.get(), write_termination='\r\n', read_termination='\r\n')
            ...
            if a1[2] in ('34401A'):
                ...
                lb.see('end')
        #special keithley
        if combo1.get()[:4] == 'GPIB':
            inst_1 = rm.open_resource(combo1.get())
            data_1 = inst_1.query("*IDN?")
            a.set(data_1)
            a1 = data_1
            a1 = re.findall(r'\w+', a1)
            f.set(a1[0] + '_' + a1[4] + '_' + d + '.xlsx')
            a10.set('Keithley ' + a1[4] + ' ' + 'connected')
            lb.insert('end', a10.get())
            lb.see('end')

def connect_dmm():
    USB1 = id('34411A', r'\b0x0A07\b').connect_d()
    GPIB = id('MODEL 2000', 'REN\r\n').connect_d() 
...
def connect_fluke():
    ...
    if combo2.get()[:4] == 'ASRL':
        ...
        lb.see('end')
	#ajout ci-dessous pour gerer le GPIB
...
class call(Thread):
    ...
    def run(self):
        ...
        if a1[2] == '34401A':
            ...
            wb.save(r'C:\\Users\\vgatto\\cernbox\\02_Python\\TestPoint2\\DMM\\Protocol\\Keithley_2000\\' + f.get())
            ws['E337'] = n.get()
            ws['C339'] = e
        inst_2.write('STBY')
        time.sleep(1)
...
class message(Thread):
    ...
    def run(self):
        sem.acquire()
#        start_thread(q.put((tk.messagebox.showinfo, ('Please', self.text), {} )))
...
def start():
    ...
    if a1[2] in ('34410A', '34411A'):
        ...
        thread = reset()
        # ~0.1V,10Hz
        thread = call('ac', '0.005', 'OUT 0.005 V, 10 Hz', 'CONF:VOLT:AC 0.1', 'D47', 'E47', 'CURR:AC:DET:BAND 3', '8', '0.7')
        thread = call('ac', '0.05', 'OUT 0.05 V, 10 Hz', 'CONF:VOLT:AC 0.1', 'D48', 'E48', 'CURR:AC:DET:BAND 3', '8', '0.16')
        thread = call('ac', '0.095', 'OUT 0.095 V, 10 Hz', 'CONF:VOLT:AC 0.1', 'D49', 'E49', 'CURR:AC:DET:BAND 3', '8', '0.132')
        # ~1V,10Hz
        thread = call('ac', '0.05', 'OUT 0.05 V, 10 Hz', 'CONF:VOLT:AC 1.0', 'D50', 'E50', 'CURR:AC:DET:BAND 3', '8', '0.7')
        thread = call('ac', '0.5', 'OUT 0.5 V, 10 Hz', 'CONF:VOLT:AC 1.0', 'D51', 'E51', 'CURR:AC:DET:BAND 3', '8', '0.16')
        thread = call('ac', '0.95', 'OUT 0.95 V, 10 Hz', 'CONF:VOLT:AC 1.0', 'D52', 'E52', 'CURR:AC:DET:BAND 3', '8', '0.132')
        # ~10V,10Hz
        thread = call('ac', '0.5', 'OUT 0.5 V, 10 Hz', 'CONF:VOLT:AC 10.0', 'D53', 'E53', 'CURR:AC:DET:BAND 3', '8', '0.7')
        thread = call('ac', '5', 'OUT 5 V, 10 Hz', 'CONF:VOLT:AC 10.0', 'D54', 'E54', 'CURR:AC:DET:BAND 3', '8', '0.16')
        thread = call('ac', '9.5', 'OUT 9.5 V, 10 Hz', 'CONF:VOLT:AC 10.0', 'D55', 'E55', 'CURR:AC:DET:BAND 3', '8', '0.132')
        # ~100V,10Hz
        thread = call('ac', '5', 'OUT 5 V, 10 Hz', 'CONF:VOLT:AC 100', 'D56', 'E56', 'CURR:AC:DET:BAND 3', '8', '0.7')
        # ~0.1V,50Hz
        ...
        # ~0.1V,300kHz
        thread = call('ac', '0.005', 'OUT 0.005 V, 300000 Hz', 'CONF:VOLT:AC 0.1', 'D122', 'E122', 'CURR:AC:DET:BAND 3', '5', '11.2')
        thread = call('ac', '0.05', 'OUT 0.05 V, 300000 Hz', 'CONF:VOLT:AC 0.1', 'D123', 'E123', 'CURR:AC:DET:BAND 3', '5', '2.2')
        thread = call('ac', '0.095', 'OUT 0.095 V, 300000 Hz', 'CONF:VOLT:AC 0.1', 'D124', 'E124', 'CURR:AC:DET:BAND 3', '5', '1.726')
        # ~1V,300kHz
        ...
        if b1[1] == '5522A':
            thread = message('Switch the red wire on the Calibrator into a socket larger 2,5 А')
            thread = call('dci', '2.85', 'OUT 2.85 A', 'CONF:CURR:DC 3.0', 'C158', 'D158', 'DET:BAND 20', '5', '0.171')
            thread = call('aci', '2.85', 'OUT 2.85 A, 50 Hz', 'CONF:CURR:AC 3.0', 'D180', 'E180', 'DET:BAND 20', '8', '0.052')
    ...
    if a1[2] == '34401A':
        ...
        thread = reset()
        # ~V
        thread = call('ac', '10.0', 'OUT 10.0 V, 10 Hz', 'CONF:VOLT:AC 10.0', 'D42', 'E42', 'CURR:AC:DET:BAND 3', '8', '0.09')
        thread = call('ac', '0.01', 'OUT 0.01 V, 1 kHz', 'CONF:VOLT:AC 0.1', 'D43', 'E43', 'CURR:AC:DET:BAND 3', '8', '0.46')
    ...
    if a1[2] == '34461A':
        ...
        # 10А
        thread = message('Switch the red wire to pin 10A of the Multimeter')
        thread = reset()
        if b1[1] == '5500E':
    ...
    if a1[2] in ('34465A', '34470A'):
        ...
        thread = message('Calibration complete')
        thread = reset()
#(self, name, d1, volt1 du calib, volt2 du dmm, cell1 valeur mesuree, cell2 ecart mesure, bandwidth du dmm, time, accurancy):
    if a1[4] == '2000':
        wb = load_workbook(shablon + '\\Keithley_2000.xlsx')
        ws = wb.active
        ws.insert_rows(18)
        progress1.configure(maximum = 47)
        lb.insert('end', 'Start time:' + ' ' + d1)
        # 0.1V
        thread = call('dc', '0.1', 'OUT 0.1 V', 'CONF:VOLT:DC 0.1', 'C32', 'D32', '', '3', '0.009')
        # 1V
        thread = call('dc', '1.0', 'OUT 1.0 V', 'CONF:VOLT:DC 1.0', 'C37', 'D37', '', '3', '0.004')
        # 10V
        thread = call('dc', '10.0', 'OUT 10.0 V', 'CONF:VOLT:DC 10', 'C42', 'D42', '', '3', '0.003')
        thread = reset()
        # ~0.1V,20Hz
        thread = call('ac', '0.1', 'OUT 0.1 V, 20 Hz', 'CONF:VOLT:AC 0.1', 'D61', 'E61', 'VOLT:AC:DET:BAND 3', '8', '0.07')
        # ~1V,20Hz
        thread = call('ac', '1.0', 'OUT 1.0 V, 20 Hz', 'CONF:VOLT:AC 1.0', 'D66', 'E66', 'VOLT:AC:DET:BAND 3', '8', '0.07')
        # ~10V,20Hz
        thread = call('ac', '10', 'OUT 10 V, 20 Hz', 'CONF:VOLT:AC 10.0', 'D71', 'E71', 'VOLT:AC:DET:BAND 3', '8', '0.07')
        # ~100V,20Hz
        thread = call('ac', '10', 'OUT 10 V, 20 Hz', 'CONF:VOLT:AC 100', 'D72', 'E72', 'VOLT:AC:DET:BAND 3', '8', '0.25')
        thread = call('ac', '30', 'OUT 30 V, 20 Hz', 'CONF:VOLT:AC 100', 'D73', 'E73', 'VOLT:AC:DET:BAND 3', '8', '0.117')
        # ~0.1V,15kHz
        thread = call('ac', '0.1', 'OUT 0.1 V, 15 kHz', 'CONF:VOLT:AC 0.1', 'D81', 'E81', 'VOLT:AC:DET:BAND 3', '8', '0.07')
        # ~1V,15kHz
        thread = call('ac', '1.0', 'OUT 1.0 V, 15 kHz', 'CONF:VOLT:AC 1.0', 'D86', 'E86', 'VOLT:AC:DET:BAND 3', '8', '0.07')
        # ~10V,15kHz
        thread = call('ac', '10', 'OUT 10 V, 15 kHz', 'CONF:VOLT:AC 10.0', 'D91', 'E91', 'VOLT:AC:DET:BAND 3', '8', '0.07')
        # ~0.1V,30kHz
        thread = call('ac', '0.1', 'OUT 0.1 V, 30 kHz', 'CONF:VOLT:AC 0.1', 'D106', 'E106', 'VOLT:AC:DET:BAND 3', '8', '0.1')
        # ~1V,30kHz
        thread = call('ac', '1.0', 'OUT 1.0 V, 30 kHz', 'CONF:VOLT:AC 1.0', 'D111', 'E111', 'VOLT:AC:DET:BAND 3', '8', '0.1')
        # ~10V,30kHz
        thread = call('ac', '10', 'OUT 10 V, 30 kHz', 'CONF:VOLT:AC 10.0', 'D116', 'E116', 'VOLT:AC:DET:BAND 3', '8', '0.1')
        # ~0.1V,70kHz
        thread = call('ac', '0.1', 'OUT 0.1 V, 70 kHz', 'CONF:VOLT:AC 0.1', 'D131', 'E131', 'VOLT:AC:DET:BAND 3', '8', '0.2')
        # ~1V,70kHz
        thread = call('ac', '1.0', 'OUT 1.0 V, 70 kHz', 'CONF:VOLT:AC 1.0', 'D136', 'E136', 'VOLT:AC:DET:BAND 3', '8', '0.2')
        # ~10V,70kHz
        thread = call('ac', '10', 'OUT 10 V, 70 kHz', 'CONF:VOLT:AC 10.0', 'D141', 'E141', 'VOLT:AC:DET:BAND 3', '8', '0.2')
        thread = call('ac', '0.1', 'OUT 0.1 V, 200 kHz', 'CONF:VOLT:AC 0.1', 'D156', 'E156', 'VOLT:AC:DET:BAND 3', '8', '1.1')
        # ~1V,200kHz
        thread = call('ac', '1.0', 'OUT 1.0 V, 200 kHz', 'CONF:VOLT:AC 1.0', 'D161', 'E161', 'VOLT:AC:DET:BAND 3', '8', '1.1')
        # ~10V,100kHz
        thread = call('ac', '10', 'OUT 10 V, 100 kHz', 'CONF:VOLT:AC 10.0', 'D166', 'E166', 'VOLT:AC:DET:BAND 3', '8', '1.1')
        # 5Hz
        thread = call('fr', '5.0', 'OUT 0.1 V, 5 Hz', 'CONF:FREQ 5.0 Hz', 'C299', 'D299', '', '5', '0.07')
        # 50Hz
        thread = call('fr', '50.0', 'OUT 0.1 V, 50 Hz', 'CONF:FREQ 50.0 Hz', 'C301', 'D301', '', '5', '0.03')
        # 500Hz
        thread = call('fr', '500.0', 'OUT 0.1 V, 500 Hz', 'CONF:FREQ 500.0 Hz', 'C303', 'D303', '', '5', '0.007')
        # 100kHz
        thread = call('fr', '100000.0', 'OUT 0.1 V, 100 kHz', 'CONF:FREQ 100.0 kHz', 'C305', 'D305', '', '5', '0.007')
        # C
        thread = message('Measuring capacitance. Pull the red wire out of the calibrator to compensate for the wires')
        thread = cap()
        thread = message('Put the wire back in place')
        thread = call('cap', '0.000000001', 'OUT 1 NF', 'CONF:CAP 1 NF', 'C331', 'D331', '', '5', '1.0')
        thread = call('cap', '0.00000001', 'OUT 10 NF', 'CONF:CAP 10 NF', 'C332', 'D332', '', '5', '0.5')
        thread = call('cap', '0.0000001', 'OUT 100 NF', 'CONF:CAP 100 NF', 'C333', 'D333', '', '5', '0.5')
        thread = call('cap', '0.000001', 'OUT 1 UF', 'CONF:CAP 1 UF', 'C334', 'D334', '', '5', '0.5')
        # 0.01A
        thread = message('Switch wires to measure current')
        thread = reset()
        thread = call('dci', '0.01', 'OUT 0.01 A', 'CONF:CURR:DC 0.01', 'C185', 'D185', '', '5', '0.07')
        # 0.1A
        thread = call('dci', '0.1', 'OUT 0.1 A', 'CONF:CURR:DC 0.1', 'C190', 'D190', '', '5', '0.055')
        # 1A
        thread = call('dci', '1.0', 'OUT 1.0 A', 'CONF:CURR:DC 1.0', 'C195', 'D195', '', '5', '0.09')
        thread = reset()
        # ~0.01A, 20Hz
        thread = call('aci', '0.01', 'OUT 0.01 A, 20 Hz', 'CONF:CURR:AC 0.01', 'D220', 'E220', 'CURR:AC:DET:BAND 3', '8', '0.14')
        # ~0.1A, 20Hz
        thread = call('aci', '0.1', 'OUT 0.1 A, 20 Hz', 'CONF:CURR:AC 0.1', 'D225', 'E225', 'CURR:AC:DET:BAND 3', '8', '0.14')
        # ~1A, 20Hz
        thread = call('aci', '1.0', 'OUT 1.0 A, 20 Hz', 'CONF:CURR:AC 1.0', 'D230', 'E230', 'CURR:AC:DET:BAND 3', '8', '0.14')
        # ~0.01A, 1kHz
        thread = call('aci', '0.01', 'OUT 0.01 A, 1 kHz', 'CONF:CURR:AC 0.01', 'D241', 'E241', 'CURR:AC:DET:BAND 20', '5', '0.14')
        # ~0.1A, 1kHz
        thread = call('aci', '0.1', 'OUT 0.1 A, 1 kHz', 'CONF:CURR:AC 0.1', 'D246', 'E246', 'CURR:AC:DET:BAND 20', '5', '0.14')
        # ~1A, 1kHz
        thread = call('aci', '1.0', 'OUT 1.0 A, 1 kHz', 'CONF:CURR:AC 1.0', 'D251', 'E251', 'CURR:AC:DET:BAND 20', '5', '0.14')
        # ~0.01A, 5kHz
        thread = call('aci', '0.01', 'OUT 0.01 A, 5 kHz', 'CONF:CURR:AC 0.01', 'D272', 'E272', 'CURR:AC:DET:BAND 20', '5', '0.14')
        # ~0.1A, 5kHz
        thread = call('aci', '0.1', 'OUT 0.1 A, 5 kHz', 'CONF:CURR:AC 0.1', 'D277', 'E277', 'CURR:AC:DET:BAND 20', '5', '0.14')
        # ~1A, 5kHz
        thread = call('aci', '1.0', 'OUT 1.0 A, 5 kHz', 'CONF:CURR:AC 1.0', 'D282', 'E282', 'CURR:AC:DET:BAND 20', '5', '0.14')
        # Ohm
        thread = message('Switch wires in four-wire to measure resistance')
        thread = reset()	
        thread = call('res4', '100', 'OUT 100 OHM; ZCOMP WIRE4', 'CONF:FRES 100', 'C311', 'D311', '', '5', '0.01')
        thread = call('res4', '1000', 'OUT 1 KOHM; ZCOMP WIRE4', 'CONF:FRES 1000', 'C312', 'D312', '', '5', '0.005')
        thread = call('res4', '10000', 'OUT 10 KOHM; ZCOMP WIRE4', 'CONF:FRES 10000', 'C313', 'D313', '', '5', '0.005')
        thread = call('res4', '100000', 'OUT 100 KOHM; ZCOMP WIRE4', 'CONF:FRES 100000', 'C314', 'D314', '', '5', '0.005')
        thread = message('Switch wires in 2-wire to measure resistance')
        thread = reset()
        thread = call('res2', '1000000', 'OUT 1 MOHM; ZCOMP WIRE2', 'CONF:RES 1000000', 'C322', 'D322', '', '5', '0.075')
        thread = call('res2', '10000000', 'OUT 10 MOHM; ZCOMP WIRE2', 'CONF:RES 10000000', 'C323', 'D323', '', '5', '0.026')
        thread = call('res2', '100000000', 'OUT 100 MOHM; ZCOMP WIRE2', 'CONF:RES 100000000', 'C324', 'D324', '', '5', '0.301')
        thread = message('Calibration complete')
        thread = reset()
...
def protokol(): 
    rep = filedialog.askopenfilenames(
        parent=root,
        initialdir=r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\Protocol',
        initialfile='',
...
def about_win():
    ...
    top.iconbitmap(r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\icon.ico')
    top.resizable(0, 0)
    ...
    text1 = ('Digital Multimeter v1.07\rAutor: ITL\r\Supported Multimeters:\r34401A\r34410A\r34411A\r34460A\r34461A\r34465A\r34470A')
...
root = tkinter.Tk()
root.title('TestPoint 1.1')
root.geometry('805x430')
root.iconbitmap(r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\icon.ico')
...
main_menu.add_cascade(label='Protocol', command=protokol)
...
img1 = tk.PhotoImage(file=r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\pan2.gif')
img2 = tk.PhotoImage(file=r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\start1.gif')
img3 = tk.PhotoImage(file=r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\ref1.gif')
img4 = AnimatedGif(r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\progress.gif')
img5 = tk.PhotoImage(file=r'C:\Users\vgatto\cernbox\02_Python\TestPoint2\DMM\icon\mick.gif')
cancel_id = None
label = tk.Label(root, image=img1)
label.place(x=1, y=1)

lbf1 = ttk.LabelFrame(root, text='Identification', width=475, height=185, style='TLabelframe')
lbf1.place(x=5, y=58)
lbf2 = ttk.LabelFrame(root, text='Calibration conditions', width=475, height=80, style='TLabelframe')
lbf2.place(x=5, y=250)
lbf3 = ttk.LabelFrame(root, text='Progress', width=645, height=50, style='TLabelframe')
...
but3 = ttk.Button(root, image=img2, command=start, style='TButton')
but3.place(x=744, y=362)
but4 = ttk.Button(root, image=img3, command=pribor, style='TButton')
...
lab3 = ttk.Label(root, text='Protocol name:', style='BW.Label')
lab3.place(x=7, y=210)
lab4 = ttk.Label(root, text='Temperature:', style='BW.Label')
lab4.place(x=7, y=265)
lab5 = ttk.Label(root, text='Humidity:', style='BW.Label')
lab5.place(x=100, y=265)
lab6 = ttk.Label(root, text='Pressure:', style='BW.Label')
lab6.place(x=190, y=265)
lab7 = ttk.Label(root, text='Pool n.:', style='BW.Label')
lab7.place(x=275, y=265)
lab8 = ttk.Label(root, text='Operator:', style='BW.Label')
...
lb = tk.Listbox(root, selectmode='extended', width=49, height=18, relief='ridge')
lb.place(x=490, y=66)
